...
public class ChatServer {
    ...
    // Map에서 secretClient를 찾는다.
    // 해당 클라이언트한테만 메시지를 전송한다.
    public void sendToOne(SocketClient sender, String secretClientName, String secretMessage) {
        String key = findClientKey(secretClientName);

        if (key == null) {
            return;
        }
        SocketClient socketClient = clients.get(key);

        if (socketClient != null) {
            JSONObject json = new JSONObject();
            json.put("clientIp", sender.clientIp);
            json.put("chatName", sender.chatName);
            json.put("message", "비밀 메시지 : " + secretMessage);
            String sendData = json.toString();

            socketClient.send(sendData);
            sender.send(sendData);
            System.out.println("비밀메시지 잘 보내짐");
        }
    }

    private String findClientKey(String secretClientName) {
        // secretClientName : 재연
        Set<String> keySet = clients.keySet();
        Iterator<String> iterator = keySet.iterator();
        while (iterator.hasNext()) {
            String key = iterator.next();
            if (key.split("@")[0].equals(secretClientName)) {
                return key;
                // 재연@192.168.0.29
            }
        }
